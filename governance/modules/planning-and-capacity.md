...
Use the delivery triad: minimize safely verified critical-path time; count tokens, context, coordination, and synthesis as cost; count available delegated worker seats as resources. Use the largest useful independent set only when time compression or assurance exceeds coordination cost. For equal safe time targets, prefer lower cost and the lowest capable model and reasoning tier. Safety, isolation, dependency order, validation, repair capacity, and evidence dominate speed.
