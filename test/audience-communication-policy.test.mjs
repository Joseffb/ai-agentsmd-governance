...
test("kernel distinguishes human and agent communication without exceeding its budget", async () => {
  ...
  assert.match(text, /Limit parallelism only for coupling,\s+unsafe overlap, unavailable capacity, dependencies, or coordination benefit erased/i);
  assert.match(text, /references, safe next action/i);
  assert.match(text, /changed files/i);
  assert.match(text, /valid in-scope user instructions/i);
  assert.match(text, /resolve exact project\/repo\/roots\/branch\/worktree[\s\S]{0,100}immediate effect/i);
  assert.match(text, /retain every entered content ledger entry and estimated cost/i);
  assert.match(text, /Bind each worker: project\/repository[\s\S]{0,180}expected artifact, upstream assumptions, validation, integration order/i);
  ...
  assert.match(text, /use a safe supported path/i);
  assert.match(text, /honest residual risk[\s\S]{0,180}when relevant/i);
  assert.match(text, /genuine authority, destructive ambiguity, or absent resources/i);
  assert.match(text, /active overlay/i);
  assert.match(text, /choose only `PARALLEL`, `PIPELINED`, `SERIAL`, or `EXPLORATORY`/i);
  assert.match(text, /Exclude shared resources only for non-file mutable state/i);
  assert.match(text, /delegate the remainder/i);
