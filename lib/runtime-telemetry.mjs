...
const TOKEN_ALIASES = Object.freeze([
  Object.freeze({ canonical: "cached_input_tokens", observed: "cached_tokens" }),
  Object.freeze({ canonical: "cache_write_input_tokens", observed: "cache_write_tokens" })
]);
...
function tokenUsage(value) {
  ...
  for (const { canonical, observed } of TOKEN_ALIASES) {
    const canonicalValue = value[canonical];
    const observedValue = value[observed];
    const hasCanonical = typeof canonicalValue === "number" && Number.isSafeInteger(canonicalValue) && canonicalValue >= 0;
    const hasObserved = typeof observedValue === "number" && Number.isSafeInteger(observedValue) && observedValue >= 0;
    if (!hasCanonical && hasObserved) selected[canonical] = observedValue;
    if (hasCanonical && hasObserved && canonicalValue !== observedValue) {
      delete selected[canonical];
      aliasConflict = true;
    }
  }
  return Object.keys(selected).length > 0 ? { value: selected, aliasConflict } : null;
}
...
function tokenRecord(parsed, session) {
  ...
  return {
    ...
    usageAliasConflict: usage?.aliasConflict ?? false,
    quota
  };
}
...
function coverageForUsage(usage, aliasConflict = false) {
  return !aliasConflict && usage?.total_tokens !== undefined ? "complete" : "partial";
}
    ...
    for await (const entry of boundedJsonLines(file)) {
      ...
      if (record.usage) {
        enqueue({
          ...
          coverage_status: coverageForUsage(record.usage, record.usageAliasConflict),
          ...record.usage
        });
