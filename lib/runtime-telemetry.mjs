...
function explicitlySubagentSession(payload) {
  ...
  const subagentSource = typeof source === "string" && source.toLowerCase() === "subagent";
  const nestedSubagentSource = source && typeof source === "object" && !Array.isArray(source) && (
...
function matchingSessionMeta(parsed, projectPath, thread) {
  ...
  return {
    ...
    session_ref: typeof payload.id === "string" && payload.id.length > 0
      ? digest(payload.id)
      : undefined,
    is_subagent: explicitlySubagentSession(payload)
  };
}
...
export async function ingestRuntimeTelemetry({
  ...
  const existing = new Set(readEngineeringEvents({ ledger: targetLedger }).events.map((event) => event.event_id));
  const pending = [];
  ...
  const enqueue = (input) => {
    const event = normalizeEngineeringEvent(input);
  ...
  for (const file of sessionFiles(sessionRoot)) {
    ...
    for await (const entry of boundedJsonLines(file)) {
      ...
      if (!isSessionMeta && !isTokenCount) continue;
      let parsed;
  ...
  for (const session of earliestSubagentSessionStarts.values()) {
    enqueue({
      event_id: recordId("seat.started", session.task_id, session.occurred_at, session.seat_id),
      type: "seat.started",
      project,
      task_id: session.task_id,
    ...
    if (!current || session.occurred_at < current) incompleteSeatCoverage.set(session.task_id, session.occurred_at);
  }
