...
function agentSystemConsentContract(consent, legacy = null) {
  if (legacy) {
    return {
      ...
      automatic_repair: "declined",
      token_cost_acknowledged: "Unverified",
      migration: legacy.auto_report ? "legacy_automatic_report_migrated_to_log_only_and_repair_declined" : "legacy_reporting_disabled_and_repair_declined"
    };
  }
  if (!consent) {
    return {
      ...
      automatic_repair: "undecided",
      token_cost_acknowledged: false,
      next_actions: [
        "profile agent-system --persistent-task yes|no --automatic-defect-report yes|no --automatic-repair yes|no [--reported-defect-action log_only|auto_correct when reporting is enabled] --acknowledge-agent-system-token-cost --authorize-profile-write",
        "Persistent tasks, automatic reports, and automatic repair can consume tokens; make each choice explicitly."
      ]
    };
  }
  ...
  const missingAutomaticRepairConsent = consent.automatic_repair === undefined;
  const automaticRepair = missingAutomaticRepairConsent ? "declined" : consent.automatic_repair;
  assert(valid.includes(automaticRepair), "Automatic repair consent must be granted or declined");
  assert(!(automaticRepair === "granted" && (automaticDefectReports !== "granted" || reportedDefectAction !== "auto_correct")), "Automatic repair requires automatic defect reports with auto_correct disposition");
  return {
    decision_required: false,
    ...
    ...(migratedLegacyConsent || missingAutomaticRepairConsent ? {
      migration: [
        ...(migratedLegacyConsent ? ["legacy_automatic_report_consent_migrated_to_log_only"] : []),
        ...(missingAutomaticRepairConsent ? ["missing_automatic_repair_consent_migrated_to_declined"] : [])
      ].join("_")
    } : {})
  };
}
...
function agentSystemReportingContract(agentSystem) {
  ...
  const automaticRepairEnabled = enabled && reportedDefectAction === "auto_correct" && agentSystem.consent.automatic_repair === "granted";
  ...
  const deliveryUnavailable = {
    ...
    local_fallback_command: "agent-system record-issue --delivery-unavailable --category <explicit-category> --failure-class <explicit-class>",
    delivery_eligibility: "never_cross_task_delivery_eligible",
    required_classification: ["category", "failure_class"],
  ...
  return {
    ...
    reported_defect_action: reportedDefectAction,
    automatic_repair_enabled: automaticRepairEnabled,
    automatic_kpi_reporting_enabled: false,
    trigger: "append_every_bounded_incident_then_apply_confirmed_agent_system_new_class_reclassification_material_evidence_or_true_blocker_transition_eligibility",
    caller_action: enabled
      ? "append_the_incident_locally_then_when_report_eligibility_is_true_list_current_tasks_and_send_one_secret_free_report_to_the_exact_non_archived_label_match"
    ...
    incident_policy: "append_every_bounded_incident_and_group_by_stable_failure_class",
    duplicate_policy: "retain_repeats_locally_and_deliver_only_when_report_eligibility_allows",
    disposition: automaticRepairEnabled
      ? {
      ...
      : {
          ...
          behavior: enabled
            ? (reportedDefectAction === "auto_correct" ? "record_and_report_without_repair_until_explicit_automatic_repair_consent" : "record_and_group_without_repair")
            : "private_local_issue_jsonl",
          continuation: "continue_safe_in_scope_project_work_without_waiting_for_agent_system"
        },
    ...
    local_cli_transport: "agent-system record-issue",
...
export function configureAgentSystemProfile(options, profileFile = null) {
  ...
  // RC-3 callers predate the distinct repair decision. Preserve their valid
  // profile writes by treating its omission as an explicit safe decline.
  const automaticRepair = options?.automaticRepair ?? false;
  assert(typeof automaticRepair === "boolean", "Automatic repair choice is required; use yes or no");
  assert(options?.acknowledgeTokenCost === true, "Agent System configuration requires --acknowledge-agent-system-token-cost; all three choices can consume tokens");
  ...
  assert(!(automaticRepair && (!options.automaticDefectReport || reportedDefectAction !== "auto_correct")), "Automatic repair requires automatic defect reports with auto_correct disposition");
  ...
  profile.agent_system = {
    ...
    consent: {
      ...
      reported_defect_action: reportedDefectAction,
      automatic_repair: automaticRepair ? "granted" : "declined",
      token_cost_acknowledged: true
    }
  };
  fs.mkdirSync(path.dirname(file), { recursive: true });
...
const INCIDENT_CATEGORIES = new Set([
  ...
  "worker_adherence",
  "host_runtime",
  "project_tool_side_effect",
  "caller_error",
  "expected_fail_closed"
]);

function incidentCategory(value) {
  const category = value === undefined ? "agent_system" : assertSafeLocalIssueText(value, "Incident category", 40);
  assert(INCIDENT_CATEGORIES.has(category), `Incident category is invalid. Valid values: ${[...INCIDENT_CATEGORIES].join(", ")}`);
  return category;
}

function incidentFailureClass(value, issueId) {
  const failureClass = value === undefined
    ? `legacy.${issueId.toLowerCase()}`
    : assertSafeLocalIssueText(value, "Failure class", 96);
  assert(/^[a-z0-9][a-z0-9._:-]{0,95}$/u.test(failureClass), "Failure class must be a stable lowercase slug");
  return failureClass;
}

function optionalIncidentText(value, field, maximum) {
  return value === undefined ? null : assertSafeLocalIssueText(value, field, maximum);
}

function optionalIncidentBoolean(value, field) {
  assert(value === undefined || typeof value === "boolean", `${field} must be a boolean when supplied`);
  return value === true;
}

function incidentEntryFailureClass(entry) {
  if (typeof entry.failure_class === "string") return entry.failure_class;
  if (typeof entry.issue_id === "string") return `legacy.${entry.issue_id.toLowerCase()}`;
  return null;
}

function incidentReportEligibility({ automaticReportingEnabled, deliveryUnavailable, category, categoryExplicit, confirmed, isNewConfirmedAgentSystemTransition, isConfirmedAgentSystemReclassification, isMaterialRepairEvidence, trueCoreBlockerTransition }) {
  const confirmedExplicitAgentSystem = categoryExplicit && category === "agent_system" && confirmed;
  const newConfirmedAgentSystemFailure = isNewConfirmedAgentSystemTransition && confirmedExplicitAgentSystem;
  const materiallyNewConfirmedAgentSystemEvidence = isMaterialRepairEvidence && confirmedExplicitAgentSystem;
  const reasons = [
    ...(newConfirmedAgentSystemFailure ? ["new_confirmed_agent_system_failure_class"] : []),
    ...(isConfirmedAgentSystemReclassification ? ["confirmed_agent_system_reclassification"] : []),
    ...(materiallyNewConfirmedAgentSystemEvidence ? ["materially_new_repair_evidence"] : []),
    ...(trueCoreBlockerTransition && confirmedExplicitAgentSystem ? ["true_p0_p1_core_blocker_without_supported_fallback"] : [])
  ];
  const eligible = !deliveryUnavailable && reasons.length > 0;
  return {
    eligible,
    reasons: eligible ? reasons : [],
    automatic_reporting_enabled: automaticReportingEnabled,
    cross_task_delivery_eligible: automaticReportingEnabled && eligible,
    caller_action: deliveryUnavailable
      ? "keep_the_delivery_unavailable_incident_private_and_do_not_retry_or_resend; continue_project_work"
      : automaticReportingEnabled && eligible
      ? "resolve_the_exact_non_archived_agent_system_task_at_delivery_time_then_send_one_secret_free_report_and_continue_project_work"
      : "keep_this_incident_private_in_the_local_append_only_stream_and_continue_project_work"
  };
}

function tryAcquireIncidentLock(ledger) {
  const lockFile = `${ledger}.lock`;
  try {
    const descriptor = fs.openSync(lockFile, "wx", 0o600);
    fs.fchmodSync(descriptor, 0o600);
    return { descriptor, lockFile, contended: false };
  } catch (error) {
    if (error && typeof error === "object" && error.code === "EEXIST") return { descriptor: null, lockFile, contended: true };
    throw error;
  }
}

function contendedIncidentResult(ledger, automaticReportingEnabled) {
  return {
    schema_version: 1,
    command: "agent-system record-issue",
    mode: "ledger_contention_deferred",
    recorded: false,
    ...
    report_eligibility: {
      ...
      reasons: [],
      automatic_reporting_enabled: automaticReportingEnabled,
      cross_task_delivery_eligible: false,
      caller_action: "keep_the_contended_incident_private_without_classifying_or_appending; continue_project_work_and_retry_the_same_bounded_incident_after_the_ledger_owner_releases"
    },
    auto_correction_eligibility: {
      eligible: false,
      caller_action: "do_not_start_automatic_repair_while_incident_ledger_serialization_is_unavailable; continue_project_work_through_the_supported_path"
    },
    next_action: "keep_the_contended_incident_private_without_classifying_or_appending; continue_project_work_and_retry_the_same_bounded_incident_after_the_ledger_owner_releases"
  };
}

function releaseIncidentLock(lock) {
  if (lock.descriptor === null) return;
  try {
...
function malformedTrailingRecovery(raw) {
  return {
    schema_version: 2,
    event_id: crypto.randomUUID(),
    recorded_at: new Date().toISOString(),
    event_type: "malformed_trailing_recovery",
    malformed_row_sha256: `sha256:${sha256(raw)}`,
    disposition: "preserve_the_malformed_trailing_row_and_continue_with_later_append_only_incidents"
  };
}

function readIncidentLedger(ledger) {
  if (!fs.existsSync(ledger)) return { entries: [], malformedTrailingRow: null, malformedTrailingRowTerminated: false };
  const content = fs.readFileSync(ledger, "utf8");
  if (!content) return { entries: [], malformedTrailingRow: null, malformedTrailingRowTerminated: false };
  const malformedTrailingRowTerminated = content.endsWith("\n");
  const lines = content.split("\n");
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    try {
      ...
      if (index === lines.length - 1) {
        return { entries, malformedTrailingRow: line, malformedTrailingRowTerminated };
      }
      let recovery;
      try {
        recovery = JSON.parse(lines[index + 1]);
      } catch {
        assert(false, "Agent System incident ledger contains a malformed non-tail JSONL row");
      }
      const digest = `sha256:${sha256(line)}`;
      assert(recovery?.event_type === "malformed_trailing_recovery" && recovery.malformed_row_sha256 === digest, "Agent System incident ledger contains a malformed non-tail JSONL row");
    }
  }
  return { entries, malformedTrailingRow: null, malformedTrailingRowTerminated: false };
}

function incidentAutoCorrectionEligibility({ automaticRepairEnabled, trueCoreBlocker, locallyActionable }) {
  const eligible = automaticRepairEnabled && trueCoreBlocker && locallyActionable;
  return {
    eligible,
    caller_action: eligible
      ? "automatic_repair_is_limited_to_the_confirmed_locally_actionable_private_agent_system_blocker"
      : "do_not_start_automatic_repair; continue_project_work_through_the_supported_path"
  };
}

function incidentBlockerFacts({ coreCapability, locallyActionable, supportedFallback, privateAgentSystemScope, repairAuthority, exclusionsComplete }) {
  return {
    core_capability: coreCapability,
    locally_actionable: locallyActionable,
    supported_fallback: supportedFallback,
    private_agent_system_scope: privateAgentSystemScope,
    repair_authority: repairAuthority,
    exclusions_complete: exclusionsComplete
  };
}

function incidentEntryBlockerFacts(entry) {
  return incidentBlockerFacts({
    coreCapability: entry.core_capability === true,
    locallyActionable: entry.locally_actionable === true,
    supportedFallback: entry.supported_fallback,
    privateAgentSystemScope: entry.private_agent_system_scope === true,
    repairAuthority: entry.repair_authority === true,
    exclusionsComplete: entry.exclusions_complete === true
  });
}
...
export function recordLocalAgentSystemIssue(options, profileFile = null) {
  ...
  if (evidenceClass) assert(["Observed", "Verified", "Inferred", "Proposed", "Unknown", "Unverified"].includes(evidenceClass), "Evidence class is invalid");
  ...
  const categoryExplicit = options?.category !== undefined;
  const requestedCategory = categoryExplicit ? incidentCategory(options.category) : null;
  const failureClassExplicit = options?.failureClass !== undefined;
  const failureClass = incidentFailureClass(options?.failureClass, issueId);
  const correctionOf = optionalIncidentText(options?.correctionOf, "Correction of", 96);
  const reclassifiedFromCategory = options?.reclassifiedFromCategory === undefined ? null : incidentCategory(options.reclassifiedFromCategory);
  const materialRepairEvidenceRequested = optionalIncidentBoolean(options?.materialRepairEvidence, "Material repair evidence");
  const coreCapability = optionalIncidentBoolean(options?.coreCapability, "Core capability");
  const locallyActionable = optionalIncidentBoolean(options?.locallyActionable, "Locally actionable");
  const privateAgentSystemScope = optionalIncidentBoolean(options?.privateAgentSystemScope, "Private Agent System scope");
  const repairAuthority = optionalIncidentBoolean(options?.repairAuthority, "Repair authority");
  const exclusionsComplete = optionalIncidentBoolean(options?.exclusionsComplete, "Complete exclusions");
  const supportedFallback = options?.supportedFallback;
  assert(supportedFallback === undefined || typeof supportedFallback === "boolean", "Supported fallback must be a boolean when supplied");
  const hasStructuredBlockerFactInput = ["coreCapability", "locallyActionable", "supportedFallback", "privateAgentSystemScope", "repairAuthority", "exclusionsComplete"].some((field) => options?.[field] !== undefined);
  assert(!materialRepairEvidenceRequested || Boolean(evidence), "Material repair evidence requires bounded evidence");
  assert(!reclassifiedFromCategory || categoryExplicit, "Reclassification requires an explicit target category");
  assert(!deliveryUnavailable || (categoryExplicit && failureClassExplicit), "--delivery-unavailable requires explicit --category and --failure-class");
  ...
  const incidentLock = tryAcquireIncidentLock(ledger);
  if (incidentLock.contended) return contendedIncidentResult(ledger, automaticReportingEnabled);
  try {
  const { entries: existing, malformedTrailingRow, malformedTrailingRowTerminated } = readIncidentLedger(ledger);
  const classEntries = existing.filter((entry) => incidentEntryFailureClass(entry) === failureClass);
  const latestEffectiveCategory = [...classEntries].reverse().find((entry) => typeof entry.category === "string")?.category ?? null;
  if (categoryExplicit && latestEffectiveCategory && requestedCategory !== latestEffectiveCategory) {
    assert(reclassifiedFromCategory === latestEffectiveCategory, "Changing an incident category requires explicit reclassification from the current category");
  }
  if (reclassifiedFromCategory && latestEffectiveCategory) {
    assert(requestedCategory !== latestEffectiveCategory, "Reclassification must change the current incident category");
  }
  const category = categoryExplicit ? requestedCategory : latestEffectiveCategory;
  const classificationStatus = categoryExplicit
    ? (reclassifiedFromCategory ? "explicit_reclassification" : "explicit")
    : latestEffectiveCategory
      ? "unclassified_inherited_local_only"
      : "unclassified_local_only";
  const hasLegacyClassEntry = classEntries.some((entry) => entry.schema_version === 1);
  const isNewClass = classEntries.length === 0;
  const evidenceDigest = evidence ? `sha256:${sha256(evidence)}` : null;
  const hasNewEvidence = Boolean(evidenceDigest) && !classEntries.some((entry) => (entry.evidence_digest === evidenceDigest || (!entry.evidence_digest && entry.evidence === evidence)) && entry.evidence_class === evidenceClass);
  const blockerFacts = incidentBlockerFacts({ coreCapability, locallyActionable, supportedFallback, privateAgentSystemScope, repairAuthority, exclusionsComplete });
  const hasNewBlockerFacts = hasStructuredBlockerFactInput && !classEntries.some((entry) => canonicalJson(incidentEntryBlockerFacts(entry)) === canonicalJson(blockerFacts));
  const eventType = correctionOf || reclassifiedFromCategory
    ? "correction"
    : isNewClass
      ? "new_class"
      : hasNewEvidence || hasNewBlockerFacts
        ? "addendum"
        : "repeat";
  const isMaterialRepairEvidence = eventType === "addendum" && materialRepairEvidenceRequested && hasNewEvidence && !hasLegacyClassEntry;
  const confirmed = evidenceClass === "Observed" || evidenceClass === "Verified";
  const hadPriorConfirmedAgentSystemState = classEntries.some((entry) => entry.category === "agent_system" && entry.category_explicit === true && ["Observed", "Verified"].includes(entry.evidence_class));
  const isNewConfirmedAgentSystemTransition = eventType !== "correction" && !hasLegacyClassEntry && category === "agent_system" && categoryExplicit && confirmed && !hadPriorConfirmedAgentSystemState;
  const isConfirmedAgentSystemReclassification = eventType === "correction" && reclassifiedFromCategory !== null && reclassifiedFromCategory !== "agent_system" && category === "agent_system" && categoryExplicit && confirmed && !hadPriorConfirmedAgentSystemState;
  const trueCoreBlocker = eventType !== "correction" && category === "agent_system" && categoryExplicit && confirmed && ["P0", "P1"].includes(severity) && coreCapability && locallyActionable && supportedFallback === false && privateAgentSystemScope && repairAuthority && exclusionsComplete;
  // A correction may establish an Agent System classification and report it,
  // but it is never repairable. Its unchanged successor is a repeat, not a
  // newly discovered blocker transition.
  const trueCoreBlockerTransition = trueCoreBlocker
    && eventType !== "repeat"
    && !classEntries.some((entry) => entry.true_core_blocker === true && entry.event_type !== "repeat");
  const autoCorrectionEligibility = incidentAutoCorrectionEligibility({
    automaticRepairEnabled: !deliveryUnavailable && automaticReportingEnabled && normalized?.consent?.reported_defect_action === "auto_correct" && normalized?.consent?.automatic_repair === "granted",
    trueCoreBlocker: trueCoreBlockerTransition && categoryExplicit,
    locallyActionable
  });
  const reportEligibility = incidentReportEligibility({
    automaticReportingEnabled,
    deliveryUnavailable,
    category,
    categoryExplicit,
    confirmed,
    isNewConfirmedAgentSystemTransition,
    isConfirmedAgentSystemReclassification,
    isMaterialRepairEvidence,
    trueCoreBlockerTransition
  });
  const groupId = `sha256:${sha256(canonicalJson({ failure_class: failureClass }))}`;
  const fingerprint = sha256(canonicalJson({ failure_class: failureClass, category, severity, summary, ...(evidenceClass ? { evidence_class: evidenceClass } : {}), ...(evidence ? { evidence } : {}) }));
  const incident = {
    schema_version: 2,
    event_id: crypto.randomUUID(),
    recorded_at: new Date().toISOString(),
    ...
    group_id: groupId,
    occurrence: classEntries.length + 1,
    fingerprint,
    project,
    issue_id: issueId,
    failure_class: failureClass,
    failure_class_explicit: failureClassExplicit,
    category,
    ...
    ...(evidence ? { evidence } : {}),
    ...(evidenceDigest ? { evidence_digest: evidenceDigest } : {}),
    ...(correctionOf ? { correction_of: correctionOf } : {}),
    ...(reclassifiedFromCategory ? { reclassified_from_category: reclassifiedFromCategory } : {}),
    ...(eventType === "correction" && reclassifiedFromCategory ? { correction_kind: "reclassification" } : {}),
    ...(isMaterialRepairEvidence ? { materially_new_repair_evidence: true } : {}),
    blocker_facts: blockerFacts,
    ...(coreCapability ? { core_capability: true } : {}),
    ...(locallyActionable ? { locally_actionable: true } : {}),
    ...(supportedFallback !== undefined ? { supported_fallback: supportedFallback } : {}),
    ...(privateAgentSystemScope ? { private_agent_system_scope: true } : {}),
    ...(repairAuthority ? { repair_authority: true } : {}),
    ...(exclusionsComplete ? { exclusions_complete: true } : {}),
    true_core_blocker: trueCoreBlocker,
    auto_correction_eligibility: autoCorrectionEligibility,
    report_eligibility: reportEligibility,
    ...(isConfirmedAgentSystemReclassification ? { confirmed_agent_system_reclassification: true } : {}),
    ...(malformedTrailingRow ? { malformed_trailing_row_preserved: true } : {}),
    ...(isNewConfirmedAgentSystemTransition ? { new_confirmed_agent_system_transition: true } : {}),
    ...(deliveryUnavailable ? { delivery_unavailable: true, disposition: "record_bounded_private_jsonl_and_continue_safe_in_scope_project_work" } : {})
  };
  const descriptor = fs.openSync(ledger, "a", 0o600);
  try {
    fs.fchmodSync(descriptor, 0o600);
    const recovery = malformedTrailingRow ? malformedTrailingRecovery(malformedTrailingRow) : null;
    fs.writeSync(descriptor, `${recovery ? `${malformedTrailingRowTerminated ? "" : "\n"}${JSON.stringify(recovery)}\n` : ""}${JSON.stringify(incident)}\n`, null, "utf8");
  ...
  return {
    ...
    report_eligibility: reportEligibility,
    auto_correction_eligibility: autoCorrectionEligibility,
    malformed_trailing_row_preserved: Boolean(malformedTrailingRow),
    local_ledger: ledger,
    next_action: reportEligibility.caller_action
  };
  } finally {
    releaseIncidentLock(incidentLock);
  }
}
...
// Mutation remains root-bound.  This helper is deliberately narrower than the
// read-only lineage exception: it proves that a canonical repository is the
// exact registered mutation root for one project before a high-level seat
// workflow asks Git to create an isolated child worktree elsewhere.
export function verifyRegisteredMutationRepository({ project, repository }) {
  const requested = stableProjectSlug(project);
  const repositoryInput = assertAbsoluteWithoutTraversal(repository, "Repository");
  assert(fs.existsSync(repositoryInput), `Repository is unreadable: ${repositoryInput}`);
  assert(!fs.lstatSync(repositoryInput).isSymbolicLink(), `Repository must not be a symlink: ${repositoryInput}`);
  const resolvedRepository = fs.realpathSync(gitReadOnly(repositoryInput, ["rev-parse", "--show-toplevel"]));
  assert(resolvedRepository === repositoryInput, "Repository must be the canonical Git worktree root");
  const profile = readMachineProfile();
  const matches = [];
  for (const [candidate, roots] of Object.entries(profile.project_roots ?? {})) {
    if (!Array.isArray(roots)) continue;
    for (const root of roots) {
      const configured = assertAbsoluteWithoutTraversal(root, "Machine project root");
      if (!fs.existsSync(configured) || fs.lstatSync(configured).isSymbolicLink()) continue;
      if (fs.realpathSync(configured) === resolvedRepository) matches.push(stableProjectSlug(candidate));
    }
  }
  const projects = [...new Set(matches)].sort();
  assert(projects.length === 1, projects.length === 0
    ? `Repository is not an exact registered mutation root for project: ${requested}`
    : `Repository has ambiguous registered project lineage: ${projects.join(", ")}`);
  assert(projects[0] === requested, `Repository belongs to unrelated project: ${projects[0]}`);
  return { project: requested, repository: resolvedRepository, provenance: "exact_registered_mutation_repository" };
}

// A current helper receipt names its claimed project, but the router must still
// prove that claim against the exact registered repository. Legacy receipts
// predate the field and require unique repository/worktree profile provenance.
...
function registeredSubagentWorktree(repository, worktree, branch, head, label = "Subagent worktree") {
  ...
  assert(matches.length === 1, `${label} is not registered by the canonical repository`);
  const registered = matches[0];
  assert(registered.HEAD?.toLowerCase() === head, `${label} HEAD does not match its canonical repository registration`);
  assert(registered.branch === `refs/heads/${branch}`, `${label} branch does not match its canonical repository registration`);
  return registered;
}
...
function verifySubagentWorktreeCapability(receiptFile, requestProject) {
  ...
  const repositoryInput = assertAbsoluteWithoutTraversal(receipt.repository, "Subagent repository");
  const worktreeInput = assertAbsoluteWithoutTraversal(receipt.worktree, "Subagent worktree");
  assert(fs.existsSync(repositoryInput) && !fs.lstatSync(repositoryInput).isSymbolicLink(), "Subagent repository must be a canonical non-symlink path");
  assert(fs.existsSync(worktreeInput) && !fs.lstatSync(worktreeInput).isSymbolicLink(), "Subagent worktree must be a canonical non-symlink path");
  const repository = fs.realpathSync(repositoryInput);
  const worktree = fs.realpathSync(worktreeInput);
  ...
  registeredSubagentWorktree(repository, worktree, branch, head);
  ...
  const repositoryProvenance = verifyRegisteredMutationRepository({
    project: projectProvenance.project,
    repository
  });
  ...
  return {
    ...
    seat: receipt.seat,
    project: projectProvenance.project,
    project_provenance: projectProvenance.provenance_class,
    repository_provenance: repositoryProvenance.provenance,
    authority: "receipt_bound_worktree"
  };
}

function approvedRoots(policyRoot, project, mode, readRootEffectsAllowed) {
  ...
  const configured = [
    ...
    ...(readRootEffectsAllowed && (mode === "read_only" || mode === "delegation" || mode === "deep_audit") && Array.isArray(profile.project_read_roots?.[project])
...
export function projectRootOverlapEvidence(target) {
  const candidate = canonicalPathForComparison(assertAbsoluteWithoutTraversal(target, "Target path"));
  const profile = readMachineProfile();
  const matches = [];
  const add = (kind, project, roots) => {
    if (!Array.isArray(roots)) return;
    for (const root of roots) {
      const resolved = canonicalPathForComparison(assertAbsoluteWithoutTraversal(root, "Machine profile root"));
      if (pathWithin(candidate, resolved) || pathWithin(resolved, candidate)) matches.push({ kind, ...(project ? { project } : {}) });
    }
  };
  add("approved_root", null, profile.approved_roots);
  for (const [project, roots] of Object.entries(profile.project_roots ?? {})) add("project_root", project, roots);
  for (const [project, roots] of Object.entries(profile.project_read_roots ?? {})) add("project_read_root", project, roots);
  return Object.freeze({ target: candidate, matches, projects: [...new Set(matches.flatMap((item) => item.project ? [item.project] : []))].sort() });
}
...
function readOnlyEffects(effects) {
  return effects.every((effect) => ["none", "read_only", "claim", "continuity"].includes(effect));
}

function sensitiveReadOnlyRoot(target) {
  const home = canonicalPathForComparison(os.homedir());
  const sensitive = [".ssh", ".aws", ".codex"].map((name) => path.join(home, name));
  return sensitive.find((root) => pathWithin(target, root)) ?? null;
}

function gitReadOnlyMaybe(root, args) {
...
function gitWorktreeLineage(target) {
  const requested = assertAbsoluteWithoutTraversal(target, "Read-only Git target");
  assert(fs.existsSync(requested), `Read-only Git target is unreadable: ${requested}`);
  assert(!fs.lstatSync(requested).isSymbolicLink(), `Read-only Git target must not be a symlink: ${requested}`);
  const gitDirectory = fs.statSync(requested).isDirectory() ? requested : path.dirname(requested);
  const topLevelValue = gitReadOnlyMaybe(gitDirectory, ["rev-parse", "--show-toplevel"]);
  if (topLevelValue === null) return null;
  const topLevel = fs.realpathSync(topLevelValue);
  const resolved = canonicalPathForComparison(requested);
  assert(pathWithin(resolved, topLevel), `Read-only Git target escapes its verified worktree: ${requested}`);
  const commonDirectory = subagentCommonGitDirectory(topLevel);
  return { requested: resolved, worktree: topLevel, common_directory: commonDirectory };
}

function configuredGitLineage(profile) {
  const bindings = [];
  const add = (project, roots) => {
    if (!Array.isArray(roots)) return;
    for (const root of roots) {
      const configured = canonicalPathForComparison(assertAbsoluteWithoutTraversal(root, "Machine profile root"));
      if (!fs.existsSync(configured)) continue;
      const worktreeValue = gitReadOnlyMaybe(configured, ["rev-parse", "--show-toplevel"]);
      if (worktreeValue === null) continue;
      const worktree = fs.realpathSync(worktreeValue);
      bindings.push({ project, root: configured, worktree, common_directory: subagentCommonGitDirectory(worktree) });
    }
  };
  for (const [project, roots] of Object.entries(profile.project_roots ?? {})) add(project, roots);
  for (const [project, roots] of Object.entries(profile.project_read_roots ?? {})) add(project, roots);
  return bindings;
}

function verifiedReadOnlyGitAdmission(target, request, effects, profile) {
  if (!(["read_only", "deep_audit"].includes(request.mode)) || request.mutation_authority || !readOnlyEffects(effects)) return null;
  const lineage = gitWorktreeLineage(target);
  if (!lineage) return null;
  const sensitive = sensitiveReadOnlyRoot(lineage.worktree);
  assert(!sensitive, `Read-only Git target is inside a sensitive root: ${sensitive}`);

  const matching = configuredGitLineage(profile)
    .filter((binding) => binding.common_directory === lineage.common_directory);
  const projects = [...new Set(matching.map((binding) => binding.project).filter(Boolean))].sort();
  assert(projects.length <= 1, `Read-only Git target has ambiguous registered project lineage: ${projects.join(", ")}`);
  if (projects.length === 1) {
    assert(projects[0] === request.project, `Read-only Git target belongs to unrelated project: ${projects[0]}`);
    if (request.mode === "deep_audit") {
      const canonicalRepositories = [...new Set(matching
        .filter((binding) => binding.root === binding.worktree)
        .map((binding) => binding.worktree))];
      assert(canonicalRepositories.length === 1, "Deep audit external Git target requires one registered canonical repository");
      const branch = gitReadOnly(lineage.worktree, ["branch", "--show-current"]);
      const head = gitReadOnly(lineage.worktree, ["rev-parse", "HEAD"]).toLowerCase();
      assert(branch, "Deep audit external Git target must be on a registered branch");
      registeredSubagentWorktree(canonicalRepositories[0], lineage.worktree, branch, head, "Deep audit external Git target");
      return { ...lineage, provenance: "profile_registered_git_worktree", project: projects[0], branch, head, repository: canonicalRepositories[0] };
    }
    return { ...lineage, provenance: "profile_git_lineage", project: projects[0] };
  }

  if (request.mode === "deep_audit") fail("Deep audit external Git target requires a registered canonical repository");

  // An unbound repository is inspectable only when the caller names its
  // verified root exactly.  Descendant paths need an explicit profile binding,
  // preventing an arbitrary directory request from becoming a broad grant.
  if (lineage.requested !== lineage.worktree) return null;
  return { ...lineage, provenance: "explicit_unbound_git_repository" };
}
...
function classifyAndAuthorize(request, manifest, policyRoot, additionalApprovedRoots = []) {
  ...
  const profile = readMachineProfile();
  const gitReadOnlyLineage = [];
  const roots = [
    ...approvedRoots(policyRoot, request.project, request.mode, effects.every((effect) => !forbidden.has(effect))),
  ...
  for (const target of request.paths) {
    ...
    const gitAdmission = withinApprovedRoot || isBootstrapAlias
      ? null
      : verifiedReadOnlyGitAdmission(target, request, effects, profile);
    assert(
      withinApprovedRoot || gitAdmission || approvedBootstrapRead(target, roots[0], request.mode, effects),
    ...
    if (gitAdmission) {
      gitReadOnlyLineage.push(gitAdmission);
    }
  }
  ...
  return {
    ...
    ...(gitReadOnlyLineage.length ? { git_read_only_lineage: gitReadOnlyLineage } : {})
  };
}
...
export function resolveRoute(input, policyRoot = resolvePolicyRoot()) {
  ...
  const subagentWorktree = request.subagent_worktree_receipt
    ? verifySubagentWorktreeCapability(request.subagent_worktree_receipt, request.project)
    : null;
...
function validateMandatoryPluginMarketplace(releaseRoot, mandatoryPlugins) {
  const manifestFile = path.join(releaseRoot, ".agents", "plugins", "marketplace.json");
  assert(fs.existsSync(manifestFile), "Mandatory plugin marketplace manifest is missing from the immutable release");
  const marketplace = readJson(manifestFile);
  assert(typeof marketplace.name === "string" && marketplace.name.length > 0, "Mandatory plugin marketplace name is invalid");
  assert(Array.isArray(marketplace.plugins), "Mandatory plugin marketplace plugins are invalid");
  const names = marketplace.plugins.map((plugin) => plugin?.name).sort();
  assert(canonicalJson(names) === canonicalJson(mandatoryPlugins), "Mandatory plugin marketplace inventory is inconsistent");
  for (const plugin of marketplace.plugins) {
    assert(
      plugin?.source?.source === "local" && plugin.source.path === `./plugins/${plugin.name}`,
      `Mandatory plugin marketplace source is invalid: ${plugin?.name ?? "unknown"}`
    );
  }
  return marketplace;
}
...
export function buildRelease(sourceRoot = codeRoot, requestedId = null) {
  ...
  try {
    ...
    copyFileLocked(path.join(sourceRoot, ".agents", "plugins", "marketplace.json"), path.join(staging, ".agents", "plugins", "marketplace.json"));
    ...
    const payload = {
      ...
      mandatory_bundled_plugins: bundledSourcePlugins,
...
export function verifyRelease(releaseRoot) {
  ...
  const pluginActivationKeys = ["mandatory_bundled_plugins"];
  ...
  const pluginActivationShape = actualKeys === canonicalJson([...legacyKeys, ...versionKeys, ...rc3Keys, ...pluginActivationKeys].sort());
  ...
  assert(legacyShape || versionedShape || rc3Shape || pluginActivationShape, "Release metadata shape is invalid");
  if (versionedShape || rc3Shape || pluginActivationShape) {
    const parsedVersion = parseSystemVersion(metadata.system_version);
    if (parsedVersion.major >= 3) assert(rc3Shape || pluginActivationShape, "Release metadata omits the display channel or bundled plugin provenance");
  ...
  if (rc3Shape || pluginActivationShape) {
  ...
  if (pluginActivationShape) {
    assert(Array.isArray(metadata.mandatory_bundled_plugins), "Mandatory bundled plugin metadata is invalid");
    assert(
      canonicalJson(metadata.mandatory_bundled_plugins) === canonicalJson(metadata.bundled_source_plugins),
      "Mandatory bundled plugin inventory is inconsistent"
    );
    validateMandatoryPluginMarketplace(releaseRoot, metadata.mandatory_bundled_plugins);
  }
...
export function installStableLinks({ sourceRoot = codeRoot, releaseId, home = os.homedir(), codexHome = process.env.CODEX_HOME ?? null, faultAt = null }) {
  ...
  const targets = [
    { id: "agents", link: path.join(codexHome ?? path.join(home, ".codex"), "AGENTS.md"), target: path.join(current, "AGENTS.md") },
    { id: "policies", link: path.join(codexHome ?? path.join(home, ".codex"), "policies"), target: path.join(current, "policies") },
    { id: "skill", link: path.join(codexHome ?? path.join(home, ".codex"), "skills", "govern-codex-policy"), target: path.join(current, "skills", "govern-codex-policy") }
  ];
...
function codexHomeForActivation(home, explicitCodexHome = null) {
  return path.resolve(explicitCodexHome ?? process.env.CODEX_HOME ?? path.join(home, ".codex"));
}

function runCodexPluginCommand(executable, args, codexHome) {
  let output;
  try {
    output = execFileSync(executable, ["plugin", ...args], {
      encoding: "utf8",
    ...
    throw new Error(`Codex plugin command failed (${["plugin", ...args].join(" ")}): ${detail || "no diagnostic output"}`);
  }
  try {
    return JSON.parse(output);
  } catch {
    throw new Error(`Codex plugin command returned invalid JSON (${["plugin", ...args].join(" ")})`);
  }
}

function assertSnapshotRootSafe(root, relative) {
  const entry = fs.lstatSync(root);
  assert(!entry.isSymbolicLink(), `Refusing plugin activation snapshot through symbolic link: ${relative}`);
  assert(
    entry.isDirectory() || entry.isFile(),
    `Refusing plugin activation snapshot of unsupported filesystem entry: ${relative}`
  );
}
...
function snapshotCodexPluginState(codexHome) {
  const transaction = path.join(codexHome, ".agent-system-activation", `plugins-${process.pid}-${crypto.randomUUID()}`);
  const targets = ["config.toml", "plugins"].map((relative) => {
    const target = path.join(codexHome, relative);
    return {
      relative,
      target,
      backup: path.join(transaction, relative),
      existed: Boolean(fs.lstatSync(target, { throwIfNoEntry: false }))
    };
  });
  ...
  try {
    for (const entry of targets) {
      ...
      assertSnapshotRootSafe(entry.target, entry.relative);
      copySnapshotTree(entry.target, entry.backup);
    }
...
function restoreCodexPluginState(snapshot) {
  for (const entry of snapshot.targets) {
    fs.rmSync(entry.target, { recursive: true, force: true });
    if (entry.existed) copySnapshotTree(entry.backup, entry.target);
  }
}

function discardCodexPluginSnapshot(snapshot) {
  fs.rmSync(snapshot.transaction, { recursive: true, force: true });
}

function installedPluginEntries(listing) {
  return Array.isArray(listing?.installed) ? listing.installed : [];
}
...
function pluginTreeDigest(root) {
  ...
  const collect = (current, relative = "") => {
    ...
    if (entry.isDirectory()) {
      for (const child of fs.readdirSync(current).sort((left, right) => left.localeCompare(right))) {
        collect(path.join(current, child), path.join(relative, child));
      }
      return;
    }
    assert(entry.isFile(), `Plugin tree contains unsupported filesystem entry: ${relative}`);
    const data = fs.readFileSync(current);
    records.push({ path: relative, sha256: sha256(data), bytes: data.length });
  };
  collect(root);
  return sha256(canonicalJson(records));
}

function immutablePluginDigests(releaseRoot, plugin, version) {
  const releases = path.dirname(releaseRoot);
  const digests = new Set();
  for (const entry of fs.readdirSync(releases, { withFileTypes: true }).sort((left, right) => left.name.localeCompare(right.name))) {
    if (!entry.isDirectory() || entry.name.startsWith(".")) continue;
    const root = path.join(releases, entry.name);
    try {
      const metadata = verifyRelease(root);
      if (!metadata.bundled_source_plugins?.includes(plugin)) continue;
      const pluginRoot = path.join(root, "plugins", plugin);
      const manifest = readJson(path.join(pluginRoot, ".codex-plugin", "plugin.json"));
      if (manifest.name === plugin && manifest.version === version) digests.add(pluginTreeDigest(pluginRoot));
    } catch {
      // Only a locally verified immutable release may contribute provenance.
    }
  }
  return digests;
}

function legacyPluginIdentity(entry, plugin, releaseRoot, codexHome) {
  if (
    typeof entry?.version !== "string" ||
    typeof entry?.pluginId !== "string" || !entry.pluginId || typeof entry?.marketplaceName !== "string" || !entry.marketplaceName
  ) return null;
  try {
    const candidate = readJson(path.join(releaseRoot, "plugins", plugin, ".codex-plugin", "plugin.json"));
    if (compareSystemVersions(entry.version, candidate.version) > 0) return null;
    const trustedDigests = immutablePluginDigests(releaseRoot, plugin, entry.version);
    if (trustedDigests.size === 0) return null;
    const roots = [
      path.join(codexHome, "plugins", "cache", entry.marketplaceName, plugin, entry.version),
      typeof entry.source?.path === "string" ? entry.source.path : null
    ].filter(Boolean);
    for (const root of roots) {
      if (!fs.existsSync(root)) continue;
      const installed = readJson(path.join(root, ".codex-plugin", "plugin.json"));
      if (
        installed.name === plugin &&
        installed.version === entry.version &&
        trustedDigests.has(pluginTreeDigest(root))
...
function pluginConflictGuidance(entry) {
  const selector = typeof entry?.pluginId === "string" && entry.pluginId ? entry.pluginId : `${entry?.name}@${entry?.marketplaceName}`;
  return `Mandatory plugin conflict for ${entry?.name ?? "unknown"}: refusing to remove an unverifiable plugin from ${entry?.marketplaceName ?? "an unknown marketplace"}. Remove or disable it explicitly with \`codex plugin remove ${selector}\`, then activate the release again.`;
}

function validateMandatoryPluginListing(entries, mandatoryPlugins) {
  const related = entries.filter((entry) =>
    mandatoryPlugins.includes(entry?.name) ||
    mandatoryPlugins.some((plugin) => typeof entry?.pluginId === "string" && entry.pluginId.startsWith(`${plugin}@`))
  );
  const selectors = new Set();
  for (const entry of related) {
    const expected = typeof entry?.name === "string" && entry.name &&
      typeof entry?.marketplaceName === "string" && entry.marketplaceName
      ? `${entry.name}@${entry.marketplaceName}`
      : null;
    assert(
      expected && entry.pluginId === expected,
      `Mandatory plugin listing integrity conflict for ${entry?.name ?? entry?.pluginId ?? "unknown"}; no plugin was removed`
    );
    assert(!selectors.has(expected), `Duplicate mandatory plugin listing for ${expected}; no plugin was removed`);
    selectors.add(expected);
  }
  return related;
}

const ACTIVATION_LOCK_STALE_MS = 5 * 60 * 1000;
...
function acquireActivationLock(lock, releaseId, scope) {
  const parent = path.dirname(lock);
  const parentEntry = fs.lstatSync(parent, { throwIfNoEntry: false });
  assert(!parentEntry?.isSymbolicLink(), `Refusing activation through symbolic-link ${scope} lock directory`);
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      ...
      assert(entry?.isDirectory() && !entry.isSymbolicLink(), "Refusing plugin activation through unsafe activation lock path");
      let owner = null;
      try { owner = readJson(path.join(lock, "owner.json")); } catch {}
      if (lockOwnerIsLive(owner)) throw new Error(`Codex plugin activation is already in progress for this ${scope} (pid ${owner.pid})`);
      const ageMs = Math.max(0, Date.now() - entry.mtimeMs);
      if (ageMs < ACTIVATION_LOCK_STALE_MS) {
        throw new Error(`Codex plugin activation lock for this ${scope} is being initialized or is not yet stale`);
      }
      ...
      try {
        ...
        if (recoveryError.code !== "ENOENT" && recoveryError.code !== "EEXIST") throw recoveryError;
      }
    }
  }
  throw new Error("Codex plugin activation lock recovery could not establish exclusive ownership");
}

function releaseActivationLock(lock) {
  try {
    const owner = readJson(path.join(lock.lock, "owner.json"));
    if (owner.lock_id === lock.owner.lock_id) {
      fs.rmSync(lock.lock, { recursive: true, force: true });
    }
  } catch {
    // Do not remove a lock that cannot be proven to be ours.
  }
}

function activateMandatoryBundledPlugins(releaseRoot, metadata, { codexHome, codexExecutable }) {
  if (!Object.hasOwn(metadata, "mandatory_bundled_plugins")) {
    return {
      result: { status: "not_declared_by_legacy_release", host_hook_activation: "Unverified", hook_trust_next_actions: hookTrustNextActions() },
      rollback: () => {},
      commit: () => {}
    };
  }
  const marketplace = validateMandatoryPluginMarketplace(releaseRoot, metadata.mandatory_bundled_plugins);
  const snapshot = snapshotCodexPluginState(codexHome);
  const selector = (plugin) => `${plugin}@${marketplace.name}`;
  try {
    const allInstalled = installedPluginEntries(runCodexPluginCommand(codexExecutable, ["list", "--json"], codexHome));
    const relatedPlugins = validateMandatoryPluginListing(allInstalled, metadata.mandatory_bundled_plugins);
    const legacyPlugins = relatedPlugins.filter((entry) =>
      entry.marketplaceName !== marketplace.name
    );
    const migrations = legacyPlugins.map((entry) => ({ entry, version: legacyPluginIdentity(entry, entry.name, releaseRoot, codexHome) }));
    const conflict = migrations.find(({ version }) => version === null);
    if (conflict) throw new Error(pluginConflictGuidance(conflict.entry));
    for (const { entry } of migrations) {
      const legacySelector = typeof entry.pluginId === "string" && entry.pluginId ? entry.pluginId : `${entry.name}@${entry.marketplaceName}`;
      runCodexPluginCommand(codexExecutable, ["remove", legacySelector, "--json"], codexHome);
    }
    const marketplaces = runCodexPluginCommand(codexExecutable, ["marketplace", "list", "--json"], codexHome);
    const currentMarketplace = (marketplaces.marketplaces ?? []).find((entry) => entry?.name === marketplace.name);
    if (currentMarketplace && !sameLocalPath(currentMarketplace.root, releaseRoot)) {
      const installed = installedPluginEntries(runCodexPluginCommand(codexExecutable, ["list", "--marketplace", marketplace.name, "--json"], codexHome));
      for (const plugin of metadata.mandatory_bundled_plugins) {
        if (installed.some((entry) => entry?.pluginId === selector(plugin) || entry?.name === plugin)) {
          runCodexPluginCommand(codexExecutable, ["remove", selector(plugin), "--json"], codexHome);
        }
      }
      runCodexPluginCommand(codexExecutable, ["marketplace", "remove", marketplace.name, "--json"], codexHome);
    }
    if (!currentMarketplace || !sameLocalPath(currentMarketplace.root, releaseRoot)) {
      const registered = runCodexPluginCommand(codexExecutable, ["marketplace", "add", releaseRoot, "--json"], codexHome);
      assert(registered.marketplaceName === marketplace.name, "Codex registered an unexpected plugin marketplace");
      assert(sameLocalPath(registered.installedRoot, releaseRoot), "Codex registered the plugin marketplace from an unexpected release");
    }
    for (const plugin of metadata.mandatory_bundled_plugins) {
      runCodexPluginCommand(codexExecutable, ["add", selector(plugin), "--json"], codexHome);
    }
    const verified = installedPluginEntries(runCodexPluginCommand(codexExecutable, ["list", "--marketplace", marketplace.name, "--json"], codexHome));
    for (const plugin of metadata.mandatory_bundled_plugins) {
      const installed = verified.find((entry) => entry?.pluginId === selector(plugin));
      assert(installed?.installed === true && installed?.enabled === true, `Mandatory Codex plugin is not installed and enabled: ${plugin}`);
      assert(sameLocalPath(installed.source?.path, path.join(releaseRoot, "plugins", plugin)), `Mandatory Codex plugin source is not the candidate release: ${plugin}`);
    }
    return {
      result: {
        status: "installed_and_verified",
        marketplace: marketplace.name,
        plugins: metadata.mandatory_bundled_plugins,
        migrated_legacy_plugins: migrations.map(({ entry, version }) => ({ plugin_id: entry.pluginId, version })),
        host_hook_activation: "Unverified",
        hook_trust_next_actions: hookTrustNextActions()
      },
      rollback: () => {
        restoreCodexPluginState(snapshot);
        discardCodexPluginSnapshot(snapshot);
      },
      commit: () => discardCodexPluginSnapshot(snapshot)
    };
  } catch (error) {
    try {
      restoreCodexPluginState(snapshot);
...
export function activateRelease(sourceRoot = codeRoot, releaseId, home = os.homedir(), options = {}) {
  ...
  const codexHome = codexHomeForActivation(home, options.codexHome);
  const activationLocks = [];
  try {
    activationLocks.push(acquireActivationLock(
      path.join(sourceRoot, ".runtime", "activation.lock"),
      releaseId,
      "source runtime"
    ));
    activationLocks.push(acquireActivationLock(
      path.join(codexHome, ".agent-system-activation", "activation.lock"),
      releaseId,
      "CODEX_HOME"
    ));
    const current = currentReleaseMetadata(sourceRoot);
    const direction = current
      ...
      : "bootstrap";
    const pluginActivation = activateMandatoryBundledPlugins(
      path.join(sourceRoot, ".runtime", "releases", releaseId),
      candidate,
      { codexHome, codexExecutable: options.codexExecutable ?? process.env.CODEX_BIN ?? "codex" }
    );
    let activation;
    try {
      activation = installStableLinks({ sourceRoot, releaseId, home, codexHome, faultAt: options.faultAt ?? null });
    } catch (error) {
      pluginActivation.rollback();
      throw error;
    }
    pluginActivation.commit();
    const agentSystem = readAgentSystemProfile(process.env.ACG_MACHINE_PROFILE || path.join(codexHome, "governance-machine-profile.json"));
    ...
    return {
      ...
      mandatory_plugin_activation: pluginActivation.result,
    ...
    for (const activationLock of activationLocks.reverse()) releaseActivationLock(activationLock);
  }
}
