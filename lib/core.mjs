...
function snapshotCodexPluginState(codexHome) {
  ...
  const targets = ["config.toml", "plugins"].map((relative) => ({
    relative,
    target: path.join(codexHome, relative),
    backup: path.join(transaction, relative),
    existed: fs.existsSync(path.join(codexHome, relative))
  }));
  fs.mkdirSync(transaction, { recursive: true, mode: 0o700 });
  for (const entry of targets) if (entry.existed) fs.cpSync(entry.target, entry.backup, { recursive: true, dereference: true });
...
function restoreCodexPluginState(snapshot) {
  for (const entry of snapshot.targets) {
    ...
    if (entry.existed) fs.cpSync(entry.backup, entry.target, { recursive: true, dereference: true });
  }
}
...
function legacyPluginIdentity(entry, plugin, releaseRoot, codexHome) {
  ...
  try {
    ...
    const manifests = [
      path.join(codexHome, "plugins", "cache", entry.marketplaceName, plugin, entry.version, ".codex-plugin", "plugin.json"),
      typeof entry.source?.path === "string" ? path.join(entry.source.path, ".codex-plugin", "plugin.json") : null
    ].filter(Boolean);
    for (const manifestFile of manifests) {
      ...
      if (
        ...
        installed.author?.name === candidate.author?.name &&
        installed.interface?.developerName === candidate.interface?.developerName &&
        compareSystemVersions(installed.version, candidate.version) <= 0
...
function activateMandatoryBundledPlugins(releaseRoot, metadata, { codexHome, codexExecutable }) {
  ...
  try {
    ...
    const legacyPlugins = allInstalled.filter((entry) =>
      metadata.mandatory_bundled_plugins.includes(entry?.name) && entry?.marketplaceName !== marketplace.name
    );
