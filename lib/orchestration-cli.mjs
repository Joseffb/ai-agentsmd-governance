...
const WORKER_MESSAGE_STATIC_PREFIX = [
  "ORCHESTRATION_WORKER_ASSIGNMENT_V2",
  "Follow only the canonical assignment JSON below. Preserve its scope and authority, return bounded assignment evidence ending with its required_final_sentinel, and do not add authority.",
...
/**
 * Creates the exact worker message from a literal, cache-stable instruction
 * prefix and a canonical volatile assignment suffix. Cache availability is not
 * an input: it cannot affect admission, routing, authority, or fallback.
 */
export function composeWorkerMessage({ assignmentCore, resultSentinel } = {}) {
  if (!isPlainObject(assignmentCore)) fail("worker assignment core must be a JSON object");
  if (typeof resultSentinel !== "string" || !resultSentinel) fail("worker result sentinel must be a non-empty string");
  return WORKER_MESSAGE_STATIC_PREFIX + canonicalJson({
    ...assignmentCore,
    required_final_sentinel: resultSentinel
  });
}

function directNativeAssignment({ seat, requestedModel, requestedReasoning, message }) {
  return Object.freeze({
    schema_version: 1,
    contract: "native_direct_worker_assignment",
    admission: "direct",
    requested_model: requestedModel,
    requested_reasoning_raw: requestedReasoning,
    actual_model: UNVERIFIED,
    actual_reasoning_raw: UNVERIFIED,
    required_capabilities: Object.freeze(["native_subagent_start", "filesystem_read", "local_runtime", "thread_coordination"]),
    start_request: Object.freeze({
      task_name: `worker-seat-${seat}`,
      fork_turns: "none",
    ...
    automatic_fallback: Object.freeze({
      ordered: true,
      stop_on_first_success: true,
      steps: Object.freeze([
        Object.freeze({ order: 1, when: "native_subagent_start_is_available", action: "start_native_subagent_with_exact_start_request" }),
        Object.freeze({ order: 2, when: "native_subagent_start_is_unavailable_or_start_fails", action: "delegate_exact_assignment_manually" }),
        Object.freeze({ order: 3, when: "manual_delegation_is_unavailable", action: "return_bounded_assignment_blocker_without_expanded_authority" })
      ])
    }),
    compatibility_diagnostics: Object.freeze({
...
function mutatingIsolationFallback({ bundlePath, bundle, worker, seat }) {
  return Object.freeze({
    schema_version: 1,
    contract: "native_mutating_worker_isolation_required",
    admission: "blocked_pending_verified_worktree_and_child_preflight",
    requested_model: worker.requested_model,
    requested_reasoning_raw: worker.requested_reasoning_raw,
    actual_model: UNVERIFIED,
    actual_reasoning_raw: UNVERIFIED,
    native_assignment: null,
    admitted_assignment: null,
    high_level_next_action: Object.freeze({
      command: "seat assign",
      project: bundle.project_identity.project,
      repository: bundle.project_identity.repository_root,
      bundle_path: bundlePath,
      worker_seat: String(seat),
      write_scopes: Object.freeze([...worker.write_scopes]),
      requested_model: worker.requested_model,
      requested_reasoning_raw: worker.requested_reasoning_raw,
      required_unbound_inputs: Object.freeze(["base", "worktree_root"]),
      child_preflight: Object.freeze({
 ...
 * pass-verbatim native launch package only when it is explicitly read-only.
 * A mutating worker must first be bound through seat assign and child
 * preflight, so this command returns that high-level preparation action rather
 * than a callable native spawn request.
 */
export function orchestrateLaunch({ bundlePath, seat: seatValue } = {}) {
  ...
  if (worker.prompt_envelope.isolation_effects === "receipt_bound_git_worktree_before_mutation") {
    return Object.freeze({
      bundle_path: resolvedBundlePath,
      bundle_digest: bundle.bundle_digest,
      execution_id: bundle.execution_id,
      ...
      ...mutatingIsolationFallback({ bundlePath: resolvedBundlePath, bundle, worker, seat })
    });
  }
  if (worker.prompt_envelope.isolation_effects !== "read_only_discovery_no_project_mutation") {
    fail("orchestrate launch worker isolation contract is invalid");
  }
  ...
  const admittedMessage = composeWorkerMessage({ assignmentCore, resultSentinel });
  const nativeAssignment = directNativeAssignment({
    seat,
    requestedModel: worker.requested_model,
    requestedReasoning: worker.requested_reasoning_raw,
  ...
  const launchPackageCore = Object.freeze({
    ...
    actual_model: UNVERIFIED,
    actual_reasoning_raw: UNVERIFIED,
    native_assignment: nativeAssignment,
