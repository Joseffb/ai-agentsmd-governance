...
const LUNA_MAX_EXCLUDED_EFFECTS = new Set([
  "contract_change",
  "database_mutation",
  "deployment",
  "destructive_operation",
...
export function recommendLowestReliableModel({ complexity = "routine", adversarial = false, effects = [], spark_gate } = {}) {
  const normalized = requiredString(complexity, "complexity");
  const normalizedEffects = normalizeEffects(effects);
  const lunaExcluded = normalizedEffects.some((effect) => LUNA_MAX_EXCLUDED_EFFECTS.has(effect));
  const matrix = adversarial || normalized === "adversarial" || normalizedEffects.includes("security")
    ? { model: "gpt-5.6-sol", reasoning: "high" }
    : normalized === "complex" || lunaExcluded ? { model: "gpt-5.6-terra", reasoning: "max" }
      ...
      : normalized === "routine" ? { model: "gpt-5.6-luna", reasoning: "max" }
...
function validateModelRecommendation(recommendation, schemaVersion, classification, topology) {
  ...
  const supported = (model === "gpt-5.6-luna" && reasoning === "max") ||
    (model === "gpt-5.3-codex-spark" && reasoning === "low") ||
    (model === "gpt-5.6-terra" && reasoning === "low") ||
    (model === "gpt-5.6-terra" && reasoning === "max") ||
    (model === "gpt-5.6-sol" && reasoning === "high");
