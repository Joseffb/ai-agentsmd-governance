...
export const ORCHESTRATION_BUNDLE_SCHEMA_VERSION = 5;
export const ORCHESTRATION_COMPOSER_VERSION = "1.3.0";
...
function normalizeReleaseIdentity(identity = {}, schemaVersion = ORCHESTRATION_BUNDLE_SCHEMA_VERSION) {
  ...
  const expectedComposerVersion = schemaVersion === 2 ? "1.0.0" : schemaVersion === 3 ? "1.1.0" : schemaVersion === 4 ? "1.2.0" : ORCHESTRATION_COMPOSER_VERSION;
...
function validateTopology(topology, recommendation, schemaVersion) {
...
function validateWorkerPromptEnvelope(envelope, worker, recommendation, schemaVersion) {
  ...
  assertExactObjectKeys(envelope, schemaVersion >= 4 ? V4_WORKER_PROMPT_ENVELOPE_FIELDS : WORKER_PROMPT_ENVELOPE_FIELDS, "bundle worker prompt envelope");
  const discovery = schemaVersion >= 4 && worker.worker_kind === "bounded_discovery";
...
function validateSeat0Decision(decision, classification, schemaVersion) {
  ...
  const decisionFields = schemaVersion >= 5 ? SEAT0_DECISION_FIELDS : LEGACY_SEAT0_DECISION_FIELDS;
  ...
  if (schemaVersion >= 5) {
    const expected = seat0Decision(classification);
...
export function validateOrchestrationBundle(bundle) {
  ...
  if (!isPlainObject(bundle) || ![2, 3, 4, ORCHESTRATION_BUNDLE_SCHEMA_VERSION].includes(bundle.schema_version) || bundle.bundle_type !== "jit_orchestration") {
  ...
  validateSeat0Decision(bundle.seat0_decision, bundle.classification, bundle.schema_version);
  ...
  if (bundle.topology) validateTopology(bundle.topology, bundle.model_recommendation, bundle.schema_version);
