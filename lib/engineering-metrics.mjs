...
function cohortEvents(events, starts, fromMs, throughMs) {
  ...
  return events.filter((event) => {
    ...
    const key = taskIdentity(event);
    return key ? keys.has(key) : timestamp >= fromMs;
  });
}

function runtimeDiagnostics(events) {
  ...
  return {
    coverage_status: unlinkedSeatLifecycle.length || toolingFallbacks.length ? "partial" : "not_applicable",
...
export function buildMetricsReport(options = {}) {
  ...
  const selectedRuntimeDiagnostics = runtimeDiagnostics(events);
