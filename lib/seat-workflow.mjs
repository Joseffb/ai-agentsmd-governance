...
import {
  ...
  projectRootOverlapEvidence,
  resolveRoute,
  runAuditWorkflow,
  verifyRegisteredMutationRepository
} from "./core.mjs";
...
import { bindOrchestrationBundleToSeat } from "./orchestration-cli.mjs";
...
function executionId({ project, workId, seat, createdAt }) {
  // This is an opaque correlation, not a project path, objective, prompt, or
  // authorization token. The private assignment persists it for the worker.
  return `execution-${crypto.createHash("sha256")
    .update(JSON.stringify([project, workId, seat, createdAt]))
    .digest("hex").slice(0, 32)}`;
}

function executionIdentity({ project, workId, seat, createdAt }) {
  const execution_id = executionId({ project, workId, seat, createdAt });
  return {
    execution_id,
    correlation_id: `correlation-${crypto.createHash("sha256").update(execution_id).digest("hex").slice(0, 32)}`,
    causation_id: `causation-${crypto.createHash("sha256").update(`root:${execution_id}`).digest("hex").slice(0, 32)}`
  };
}
...
function writeReadOnlyAssignment(root, assignment) {
  ...
  persisted.native_assignment = readOnlyNativeAssignment(
    persisted,
    file,
    persisted.admitted_assignment
  );
...
function lifecycle(options, repository, worktree, receiptPath, policyRoot) {
  ...
  const route = resolveRoute({
    ...
    subagent_worktree_receipt: receiptPath,
    prior_receipt: prior
  }, policyRoot);
...
function rollbackFreshWorktree(repository, assignment) {
  try {
    const worktree = assignment.worktree;
    if (!fs.existsSync(worktree)) return;
    if (commonGitDirectory(repository) !== commonGitDirectory(worktree)) return;
    if (fs.realpathSync(git(worktree, ["rev-parse", "--show-toplevel"])) !== worktree) return;
    if (git(worktree, ["branch", "--show-current"]) !== assignment.branch) return;
    if (git(worktree, ["status", "--porcelain=v1", "--untracked-files=all"])) return;
    execFileSync("git", ["-C", repository, "worktree", "remove", "--force", worktree], { stdio: "ignore" });
    execFileSync("git", ["-C", repository, "branch", "-D", assignment.branch], { stdio: "ignore" });
  } catch {
    // Preserve the original lifecycle error. Any non-empty or mismatched
    // candidate remains intact rather than risking an unrelated worktree.
  }
}
...
function readOnlyAssignment(options) {
  ...
  const expectedNativeAssignment = readOnlyNativeAssignment(assignment, packagePath, admittedAssignment);
  let admissionContract = "direct_native_assignment";
  if (assignment.native_assignment !== undefined) {
    if (JSON.stringify(assignment.native_assignment) !== JSON.stringify(expectedNativeAssignment)) {
      fail("Read-only assignment direct native admission contract is invalid; run a fresh seat inspect");
    }
  } else {
    // Packages emitted before direct native admission remain inspectable, but
    // their quarantine handshake is diagnostic-only and cannot establish the
    // model selected by the current official native lifecycle.
    ...
    if (JSON.stringify(assignment.native_quarantine) !== JSON.stringify(expectedNativeQuarantine)) {
      fail("Read-only assignment legacy native quarantine diagnostic is invalid; run a fresh seat inspect");
    }
    admissionContract = "legacy_quarantine_diagnostic_only";
  }
  return { packagePath, assignment, identity, project, seat, target, admittedAssignment, expectedNativeAssignment, admissionContract };
}
...
function readOnlyNativeAssignment(assignment, packagePath, admittedAssignment) {
  const requestedModel = required(assignment.requested_model, "assignment model");
  const requestedReasoning = required(assignment.requested_reasoning_raw, "assignment reasoning");
  const taskName = `read-only-${workerSeat(assignment.seat, "assignment seat")}`;
  return {
    schema_version: 1,
    contract: "native_direct_read_only_assignment",
    admission: "direct",
    requested_model: requestedModel,
    requested_reasoning_raw: requestedReasoning,
    actual_model: "Unverified",
    actual_reasoning_raw: "Unverified",
    required_capabilities: ["native_subagent_start", "filesystem_read", "local_runtime", "thread_coordination"],
    start_request: {
      task_name: taskName,
      fork_turns: "none",
      model: requestedModel,
      reasoning_effort: requestedReasoning,
      message: admittedAssignment.message
    },
    pass_start_request_verbatim: true,
    automatic_fallback: {
      ordered: true,
      ...
      steps: [
        {
          order: 1,
          when: "native_subagent_start_is_available",
          action: "start_native_subagent_with_exact_start_request"
        },
        {
          order: 2,
          when: "native_subagent_start_is_unavailable_or_start_fails",
          action: "delegate_exact_admitted_assignment_manually"
        },
        {
          order: 3,
          when: "manual_delegation_is_unavailable",
          action: "return_bounded_read_only_blocker_without_mutation"
        }
      ]
    },
    compatibility_diagnostics: {
      legacy_quarantine_attestation: "not_required_for_direct_admission",
      subagent_start_model_metadata: "active_model_slug_may_be_reported_without_requested_to_actual_binding_or_reasoning_proof",
...
function prepare(options, policyRoot) {
  ...
  verifyRegisteredMutationRepository({ project: required(options.project, "project"), repository });
  ...
  const adoptedExisting = fs.existsSync(worktree);
  if (adoptedExisting) helperArgs.push("--allow-existing-clean-worktree", "yes");
  ...
  let policy;
  try {
    policy = lifecycle(options, repository, gitAssignment.worktree, gitAssignment.receipt_path, policyRoot);
  } catch (error) {
    if (!adoptedExisting) rollbackFreshWorktree(repository, gitAssignment);
    throw error;
  }
  return {
    repository,
    ...
    seat,
    workId,
    worktree: gitAssignment.worktree,
    writeScope,
    generatedOutputScope,
    policy,
    gitAssignment
...
function assignmentPackage(options, prepared, activeReceipt, provenancePath, recovered) {
  ...
  const project = slug(options.project, "project");
  const executionBinding = options.orchestration_binding ?? null;
  const identity = executionBinding
    ? {
      execution_id: executionBinding.execution_id,
      correlation_id: executionBinding.correlation_id,
      causation_id: executionBinding.causation_id
    }
    : options.execution_identity ?? executionIdentity({ project, workId: prepared.workId, seat: prepared.seat, createdAt });
  return {
    schema_version: 1,
    package_type: "governed_seat_assignment",
    created_at: createdAt,
    project,
    project_provenance: { project, provenance_class: "receipt_project_bound" },
    objective: options.objective || null,
    thread_id: options.thread || null,
    seat: prepared.seat,
    work_id: prepared.workId,
    ...identity,
    ...(executionBinding ? { orchestration_bundle: executionBinding } : {}),
...
function projectlessReadOnlyFallback({ target, seat, model, reasoning, objective }) {
  const resultSentinel = `ACG_PROJECTLESS_READ_ONLY_RESULT:${crypto.randomBytes(8).toString("hex")}`;
  const prompt = ["NATIVE_PROJECTLESS_READ_ONLY_V1", `Objective: ${objective || "Perform a bounded read-only inspection."}`, `Exact target path: ${target}`, "Effects: filesystem read only; no mutation, profile, or wait.", "Do not report only this expected unregistered-root absence; independent Agent System or runtime failures retain normal profile-selected reporting.", `Requested model: ${model}`, `Requested reasoning: ${reasoning}`, "Actual model: Unverified", "Actual reasoning: Unverified", `Append this exact sentinel on the final line: ${resultSentinel}`].join("\n");
  const requiredCapabilities = ["native_subagent_start", "filesystem_read", "local_runtime", "thread_coordination"];
  const automaticFallback = {
    ordered: true,
    stop_on_first_success: true,
    steps: [
      { order: 1, when: "native_subagent_start_is_available", action: "start_native_subagent_with_exact_start_request" },
      { order: 2, when: "native_subagent_start_is_unavailable_or_start_fails", action: "delegate_exact_admitted_assignment_manually" },
      { order: 3, when: "manual_delegation_is_unavailable", action: "return_bounded_read_only_blocker_without_mutation" }
    ]
  };
  return {
    schema_version: 1,
    ...
    visibility: "normal",
    status: "native_read_only_fallback",
    seat_ready: false,
    native_fallback_ready: true,
    read_only: true,
    project_binding: { kind: "projectless_unbound" },
    task: seat,
    owner: seat,
    seat,
    target,
    ...
    admitted_assignment: { starts_new_turn: true, message: prompt, pass_message_verbatim: true, required_final_sentinel: resultSentinel, stale_notification_rule: "Completion without the required final sentinel is inadmissible.", close_rule: "Close only after a result carrying the required final sentinel." },
    native_read_only_fallback: {
      reason: "unregistered_generated_projectless_workspace",
      exact_target_path: target,
      objective: objective || "Perform a bounded read-only inspection of the exact target path.",
      permitted_effect: "read_only_inspection",
      mutation_authorized: false,
      machine_profile_mutation_authorized: false,
      unregistered_root_absence_agent_system_defect_report_eligible: false,
      actual_model: "Unverified",
      actual_reasoning_raw: "Unverified",
      immediate_next_action: "native_or_manual_read_only_delegation",
      wait_for_agent_system: false,
      instruction: "Delegate now only for read-only inspection of exact_target_path. Do not mutate source, Git state, machine profile, policy state, packages, caches, services, or external systems."
    },
    operator_summary: `${seat} may immediately perform a bounded native or manual read-only inspection of the exact unregistered projectless target; no machine-profile mutation is authorized.`,
    completion_sentinel: `ACG_PROJECTLESS_READ_ONLY_FALLBACK:${crypto.randomBytes(8).toString("hex")}`
  };
}
...
export function inspectSeat(options, policyRoot) {
  ...
  const model = required(options.model, "model");
  const reasoning = required(options.reasoning, "reasoning");
  const orchestrationBinding = bindOrchestrationBundleToSeat({
  ...
  if (project === "null") {
    const evidence = projectRootOverlapEvidence(target);
    if (evidence.matches.length) {
      if (evidence.projects.length === 1) fail(`registered_project_binding_required:${evidence.projects[0]}`);
      if (evidence.projects.length > 1) fail("ambiguous_registered_project_binding");
      fail("registered_root_overlap_requires_project_binding");
    }
    const home = process.env.HOME && path.isAbsolute(process.env.HOME) && fs.existsSync(process.env.HOME)
      ? fs.realpathSync(process.env.HOME)
      : os.homedir();
    const sensitive = [".ssh", ".aws", ".codex"].map((name) => path.join(home, name));
    const temporaryRoot = fs.realpathSync(os.tmpdir());
    const allowedTmp = target.startsWith(`${temporaryRoot}${path.sep}`) && /generated/i.test(path.basename(target));
    const codexRoot = path.join(home, "Documents", "Codex");
    const relativeCodex = path.relative(codexRoot, target).split(path.sep);
    const allowedCodex = relativeCodex.length === 2 && /^\d{4}-\d{2}-\d{2}$/u.test(relativeCodex[0]) && relativeCodex.every((part) => part && part !== "." && part !== "..");
    if (target === path.parse(target).root || target === home || sensitive.some((root) => target === root || target.startsWith(`${root}${path.sep}`)) || !(allowedTmp || allowedCodex)) {
      fail("projectless_unbound_target_is_not_a_narrow_generated_workspace");
    }
    return projectlessReadOnlyFallback({ target, seat, model, reasoning, objective: options.objective });
  }
  let policy;
  try {
    policy = readOnlyLifecycle(options, target, policyRoot);
  } catch (error) { throw error; }
  const childCli = childCliPath();
  const resultSentinel = `ACG_READ_ONLY_SEAT_RESULT:${crypto.randomBytes(8).toString("hex")}`;
  const createdAt = new Date().toISOString();
  const identity = orchestrationBinding
    ? {
      execution_id: orchestrationBinding.execution_id,
      correlation_id: orchestrationBinding.correlation_id,
      causation_id: orchestrationBinding.causation_id
    }
    : executionIdentity({ project, workId: `read-only-${seat}`, seat, createdAt });
  const packageValue = {
    schema_version: 1,
    ...
    seat,
    ...identity,
    ...(orchestrationBinding ? { orchestration_bundle: orchestrationBinding } : {}),
    read_only: true,
    target,
    requested_model: model,
    requested_reasoning_raw: reasoning,
  ...
  recordEngineeringEventBestEffort({
    type: "execution.allocated",
    project,
    thread_id: options.thread,
    work_id: `read-only-${seat}`,
    seat_id: seat,
    ...identity,
  ...
  return {
    ...
    target,
    assignment_package: packagePath,
    native_assignment: readOnlyNativeAssignment(packageValue, packagePath, admittedAssignment),
    child_preflight: childPreflight,
    admitted_assignment: admittedAssignment,
    child_instruction: "Start the native child directly with native_assignment.start_request; its exact assignment begins with the required child preflight.",
    operator_summary: `${seat} is ready for a direct native read-only assignment of the approved target.`,
...
export function preflightSeat(options, policyRoot) {
  ...
  const assignmentIdentity = value.assignment.execution_id
    ? {
      execution_id: value.assignment.execution_id,
      ...(value.assignment.correlation_id ? { correlation_id: value.assignment.correlation_id } : {}),
      ...(value.assignment.causation_id ? { causation_id: value.assignment.causation_id } : {})
    }
    : null;
  if (assignmentIdentity) {
    recordEngineeringEventBestEffort({
      ...
      project: value.project,
      thread_id: value.assignment.thread_id || undefined,
      work_id: `read-only-${value.seat}`,
      seat_id: value.seat,
      ...assignmentIdentity,
  ...
  recordEngineeringEventBestEffort({
    ...
    evidence_class: "observed",
    evidence_authority: "runtime_metadata",
    coverage_status: assignmentIdentity ? "complete" : "partial"
  });
  return {
    ...audit,
...
function preflightMutatingSeat(packagePath, assignment, policyRoot) {
  ...
  const startedAt = new Date().toISOString();
  const assignmentIdentity = assignment.execution_id
    ? {
      execution_id: assignment.execution_id,
      ...(assignment.correlation_id ? { correlation_id: assignment.correlation_id } : {}),
      ...(assignment.causation_id ? { causation_id: assignment.causation_id } : {})
    }
    : null;
  // This is the first local observation after the worker has proved its
  // assignment. It is deliberately not backdated to preparation time.
  if (assignmentIdentity) {
    recordEngineeringEventBestEffort({
      type: "execution.running",
      project: assignment.project,
...
export function assignSeat(options, policyRoot) {
  const repository = existingRoot(options.repository, "repository");
  const project = slug(options.project, "project");
  const seat = workerSeat(options.seat, "seat");
  const orchestrationBinding = bindOrchestrationBundleToSeat({
    bundlePath: options.bundle,
    project,
    seat,
    repository: fs.realpathSync(git(repository, ["rev-parse", "--show-toplevel"]))
  });
  const boundOptions = orchestrationBinding ? { ...options, orchestration_binding: orchestrationBinding } : options;
  const prepared = prepare(boundOptions, policyRoot);
  const provenance = {
    schema_version: 1,
    receipt_type: "seat_source_provenance",
    created_at: new Date().toISOString(),
    project: boundOptions.project,
    seat: prepared.seat,
    repository: prepared.repository,
    base_commit: prepared.base,
  ...
  Object.assign(provenance, {
    execution_id: packageValue.execution_id,
    correlation_id: packageValue.correlation_id,
    causation_id: packageValue.causation_id,
    ...(orchestrationBinding ? { orchestration_bundle: orchestrationBinding } : {})
  });
  provenance.provenance_sha256 = digest(provenance);
  ...
  recordEngineeringEventBestEffort({
    ...
    correlation_id: packageValue.correlation_id,
    causation_id: packageValue.causation_id,
    occurred_at: packageValue.created_at,
    source: "seat_workflow",
    evidence_class: "observed",
    evidence_authority: "operator_record",
    coverage_status: "complete"
...
function recoverSeatFromAssignment(options, policyRoot) {
  ...
  const derivedOptions = {
    ...
    ...(source.assignment.execution_id ? {
      execution_identity: {
        execution_id: source.assignment.execution_id,
        ...(source.assignment.correlation_id ? { correlation_id: source.assignment.correlation_id } : {}),
        ...(source.assignment.causation_id ? { causation_id: source.assignment.causation_id } : {})
      }
    } : {}),
  ...
  provenance.provenance_sha256 = digest(provenance);
  if (packageValue.execution_id) {
    provenance.execution_id = packageValue.execution_id;
    provenance.correlation_id = packageValue.correlation_id;
    provenance.causation_id = packageValue.causation_id;
    provenance.provenance_sha256 = digest(provenance);
  }
  writePrivateJsonAt(provenancePath, provenance);
  recordEngineeringEventBestEffort({
...
export function finalizeSeat(options, policyRoot) {
  ...
  if (assignment.execution_id) {
    provenance.execution_id = assignment.execution_id;
    provenance.correlation_id = assignment.correlation_id;
    provenance.causation_id = assignment.causation_id;
  } else {
    provenance.execution_evidence = "Unverified";
  }
